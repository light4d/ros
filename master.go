...
func callRosApi(calleeUri string, method string, args ...interface{}) (interface{}, error) {
	...
	if xs, ok = result.([]interface{}); !ok {
		return nil, fmt.Errorf("Malformed rosgo API result.")
	}
	if len(xs) != 3 {
		err := fmt.Errorf("Malformed rosgo API result. Length must be 3 but %d", len(xs))
		return nil, err
	}
	...
	if code != ApiStatusSuccess {
		err := fmt.Errorf("rosgo Master API call failed with code %d: %s", code, message)
		return nil, err
	}
	return value, nil
}

// Build XMLRPC ready array from rosgo API result triplet.
